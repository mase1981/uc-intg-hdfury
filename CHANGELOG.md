...
### Fixed
- **Sensor Initialization**: Fixed sensors showing "Unknown" after integration setup. Previously, initial state polling was fire-and-forget, causing entities to push UNKNOWN values before polling completed. Now polling is awaited before emitting entity updates.
- **Individual Command Timeouts**: Each poll command now has isolated error handling. A single timeout no longer aborts the entire poll, allowing other sensors to update correctly.
- **Reconnect Polling**: Added state polling after keep-alive reconnection and reboot reconnection, ensuring sensors update with real values after connection recovery.

## [0.6.1] - 2026-02-22

### Fixed
- **Reboot Reconnection**: Fixed integration not reconnecting after reboot command. Previously, sending a reboot command left the integration with a stale TCP connection, causing subsequent commands to fail with "Connection reset by peer" until manual power-on was triggered. Now the integration automatically disconnects, marks device unavailable, and actively polls for reconnection every 5 seconds for up to 60 seconds after reboot.

## [0.4.3] - 2026-01-22

### Fixed
- **System Page Grid Size**: Added grid size specification (4x8) to System page to ensure all buttons are visible on physical remote. Previously, reboot and hotplug buttons placed at higher y positions were outside the visible area due to missing grid parameter, making them inaccessible on the remote's display even though the page was created.

## [0.4.2] - 2026-01-22

### Fixed
- **System Page Always Visible**: Fixed System page (containing Reboot and Hotplug buttons) to always be created on remote entity UI pages, regardless of device features. Previously, the System page was only shown if the device had OLED, Autoswitch, or HDCP support, which prevented some models (ARCANA2, Dr.HDMI 8K) from accessing reboot and hotplug controls via UI pages.
